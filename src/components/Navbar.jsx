...
function Navbar() {
  ...
  return (
    <div
      className={`fixed z-[999] w-full px-20 py-10 flex justify-between items-center font-["Neun Montreal"] transition-transform duration-500 ${isVisible ? 'translate-y-0' : '-translate-y-full'
        }`}
      style={{
        backdropFilter: 'blur(10px)',
        backgroundColor: 'rgba(255, 255, 255, 0.2)', // Semi-transparent background
        borderBottom: '1px solid rgba(255, 255, 255, 0.3)', // Optional border for effect
      }}
    >
      <div className='logo'>
      ...
      <div className="links font-montserrat flex gap-10">
        {['Service', 'Our work', 'About us', 'Insights', 'Contact us'].map(
          (item, index) => (
            <a
              href="#"
              key={index}
              className={`max-sm:invisible xl:visible text-lg capitalize font-light ${index === 4 && 'ml-20'
                }`}
            >
              {item}
            </a>
